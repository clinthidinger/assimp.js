...
var AssimpToThreeJS = (function () {
  ...
  function triangulatePoly (indices) {
    ...
    var tris = Array(triCount)
    var orderedPointCount = indices.length
    var offsetIndex = 0
    var base0 = 0 + offsetIndex
    var base1 = 1 + offsetIndex
    var lastTri = triCount - 1
    var revVtxIdx
    var triIdx = 0
    var tri
    var nextTri

    for (revVtxIdx = orderedPointCount - 1, triIdx = 0; triIdx < triCount;) {
  ...
  function loadVertexColors ( mesh, geom, colorSetIdx) {
    var numVertices = mesh.getNumVertices()
    var vertexClrAttr = new THREE.BufferAttribute(new Float32Array(numVertices * 3), 3)
    var vertIdx
    var clr
  ...
  function loadTextureCoords (mesh, geom, index) {
    var numVertices = mesh.getNumVertices()
    var textureCoordAttr = new THREE.BufferAttribute(new Float32Array(numVertices * 2), 2)
    var vertIdx
    var uv
  ...
  function loadLight (threeScene, assimpLight) {
    ...
    var threeLight = null
    var assimpDiffuseColor = assimpLight.getColorDiffuse()
    var assimpAmbientColor = assimpLight.getColorAmbient()
    var name = assimpLight.getName()
    var pos = assimpLight.getPosition()
    var threeDiffuseColor
    var threeAmbientColor
    var target
    ...
    if (lightType === ASSIMP.aiLightSourceType.AMBIENT) {
      ...
      threeLight.angle = assimpLight.getAngleOuterCone()
      threeLight.penumbra = assimpLight.getAngleInnerCone()
      threeLight.decay = assimpLight.getAttenuationConstant()
  ...
  function loadAnimation (threeScene, animation) {
    ...
    var ticksPerSecond = animation.getTicksPerSecond()
    var numChannels = animation.getNumChannels()
    var numMeshChannels = animation.getNumMeshChannels()
  ...
  function loadNode (threeNode, assimpNode, meshArray) {
    var i = 0
    var numMeshes = assimpNode.getNumMeshes()
    var numChildren = assimpNode.getNumChildren()
    var meshIndex = 0
    var assimpChildNode = null
    var threeChildNode = null
...
//-----------------------------------------------------------------------------
  ...
  function getLightDirection (threeLight) {
    ...
    var target = threeLight.target.position
    var dir = new THREE.Vector3(target.x - pos.x, target.y - pos.y, target.z - pos.z)
    dir.normalize()
    return dir
  }
  ...
  function saveLight (assimpScene, threeLight, light) {
    ...
    var target
    var dir
    var intensity = threeLight.intensity
  ...
  function saveNode (assimpScene, assimpNode, threeNode) {
    var i = 0
    var assimpChildNode = null
    var numChildren = threeNode.children.length
    var sceneIndex
   ...
   // } else if (threeNode instanceof THREE.Animation) {
    ...
    } else if (threeNode instanceof THREE.Group) {} else if (threeNode instanceof THREE.Scene) {} else if (threeNode instanceof THREE.Object3D) {}
